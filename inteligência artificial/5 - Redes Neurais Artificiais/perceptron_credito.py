	...
	def treinar(self):
		...
		while True:
			...
			if aprendeu or geracoes > self.geracoes:
				print('\nQuantidade de gerações para aprender: %d' % geracoes)
				break
...
with open('creditos.csv', mode='r', encoding='utf-8') as arquivo:
    leitor = csv.reader(arquivo)
    for linha in leitor:
        x = [float(a) for a in linha[:-1]]
        y = int(linha[-1])
...
while (True):
	...
	x = float(input('\nValor para Histórico de Crédito: '))
	y = float(input('Valor para Renda Mensal: '))
	print(x , ' , ', y)
	rede.teste([x,y])
